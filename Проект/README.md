...
Для коммутаторов spine выбрана AS 65000, для коммутаторов leaf - соответственно по их номерам 65001, 65002, 65003. Дополнительно для коммутатора ext настроена AS 65050.

Сеть строится по принципу CLOS, состоящему из двух уровней:

+ **Leaf** – коммутаторы, к которым подключается оконечное оборудование и внешние сети;
+ **Spine** – коммутаторы, которые организуют высокоскоростное ядро, обеспечивая связность между Leaf.

Преимущества архитектуры в ЦОДах:
...
+ возможность организации изолированных сетей для клиентов.

Для маршрутизации в underlay я выбрал eBGP (между Spine и Leaf), так как он:

+ используется в нашей компании и нужно потренироваться;
+ я получаю таким образом больше практики с BGP;
...
6. Проверка связности между клиентскими устройствами утилитой **ping**.
